...
async def filter_by_liquidez(
    ...
    async def _check(sym: str) -> Optional[str]:
        ...
        async with sem:
            try:
                vol, oi = await api.obter_liquidez(sym)
    ...
    if failed_syms:
        logger.info(f"Símbolos excluídos por liquidez insuficiente ou erro: {failed_syms}")
    return passed


def calculate_resistance_h1(df: pd.DataFrame) -> float:
...
def check_context(df: pd.DataFrame) -> bool:
    """
...
def check_trigger(
    ...
    try:
        ...
        # MACD
        macd_line, signal = calculate_macd(df)
